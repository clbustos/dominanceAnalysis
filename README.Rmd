...
We performed dominance analysis on this dataset and the results are shown below. The fit indexes  were *r2.m* ($R^2_M$: McFadden's measure), *r2.cs* ($R^2_{(y,\hat{y})}$: squared correlation between the predicted and the observed values of the response), *r2.n* ($R^2_N$: Nagelkerke's measure) and *r2.e* ($R^2_E$: Estrella's measure). For all fit indexes, we can conclude that *age* and *alcohol* completely dominate *tobacco*, while *age* shows general dominance over both *alcohol* and *tobacco.*
